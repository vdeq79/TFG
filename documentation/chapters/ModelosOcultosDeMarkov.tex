...
\begin{itemize}
    ...
    \begin{equation*}
        \delta_0(i)=b_i(O_0)\cdot\pi_i, \qquad 1\leq i\leq N.
    \end{equation*}
...
Con esta normalización, presentamos los pseudo-códigos que resuelven los \textbf{problema 1} y \textbf{3}: \medskip
...
\Ensure{$logProb=\log\left(P[\mathcal{Y}_0^r=O|\lambda]\right)$}
    ...
    \For{$i\gets1$ \textbf{to} $N$}
        \State $\alpha_0(i)=\pi_i\cdot b_i(O_0)$
    ...
    \For{$t\gets1$ \textbf{to} $r$}
        ...
        \For{$i\gets1$ \textbf{to} $N$}
            ...
            \EndFor
            \State $\alpha_t(i) \gets \alpha_t(i)\cdot b_i(O_t)$\;
    ...
    \For{$t\gets r-1$ \textbf{to} $0$ \textbf{by} $-1$}
        \For{$i\gets1$ \textbf{to} $N$}
                ...
                \For{$j\gets1$ \textbf{to} $N$}
                    \State$\beta_t(i)\gets \beta_t(i)+a_{ij}\cdot b_j(O_{t+1})\cdot\beta_{t+1}(j)$
                \EndFor \State
...
\While{$iters < maxIters$ y $logProb>oldLogProb$}
    ...
    \For{$t\gets0$ \textbf{to} $r-1$}
        \For{$i\gets1$ \textbf{to} $N$}
            ...
            \For{$j\gets1$ \textbf{to} $N$}
                \State $\xi_t(i,j)\gets \alpha_t(i)\cdot a_{ij}\cdot b_j(O_{t+1})\cdot \beta_{t+1}(j)$
...
\begin{section}{Reconocimiento de estados}
Recordemos que el algoritmo de Baum-Welch se utiliza para reestimar los parámetros de $\lambda=(A,B,\pi)$ con la intención de maximizar $P[\mathcal{Y}_0^r=O|\lambda]$ con $O=(O_0,\cdots,O_r)$ una secuencia de observaciones conocida. Ante problemas en los que no tenemos suficiente información, podemos partir de un modelo inicial estimado y aplicar el algoritmo de Baum-Welch para construir un modelo ajustado mediante una secuencia de observaciones suficientemente amplia. A partir de los resultados, podemos dar un significado físico a los estados del espacio $\mathcal{S}$ al que previamente sólo conocíamos su cardinalidad.


%Pero también podemos usarlo para determinar los estados de un espacio $\mathcal{S}$ al que sólo se conoce la cardinalidad, mediante una secuencia de observaciones suficientemente amplia y la aplicación del algoritmo de Baum-Welch, somos capaces de dar un significado físico a los estados.

%Antes de pasar al siguiente capítulo, vamos a presentar una de las aplicaciones clásicas de HMM. Ya habíamos mencionado la utilidad de los HMM en el procesamiento del lenguaje, en este caso, veamos un ejemplo de aplicación sobre la lengua inglesa. 

Como ejemplo, consideramos el trabajo de Cave y Neuwirth \cite{Cave}, el cuál pretende estudiar las propiedades básicas del inglés. Para ello, tomaron el libro \enquote{Brown Corpus} con alrededor de un millón de palabras. Eliminaron las puntuaciones, números, caracteres especiales y convierteron todas las letras a minúscula obteniendo $26$ letras y el espacio, un total de 27 símbolos al que consideraron como posibles observaciones.

Tomando espacios de estados $\mathcal{S}$ de cardinalidad $N=2,\dots,12$ sin especificar los significados de los estados, la aplicación del algoritmo de Baum-Welch les ha permitido llegar a conclusiones dependiendo de $N$. En particular, para $N=2$ obtuvieron las siguientes probabilidades de observar cada letra estando en uno de los dos estados:
