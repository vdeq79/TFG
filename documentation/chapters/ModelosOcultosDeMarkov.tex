...
\section{Los tres problemas básicos de los HMM}
...
\begin{itemize}
    \item En el instante inicial:
    \begin{equation*}
        \hat{\delta}_0(i)=\log(b_i(O_0)\cdot\pi_i), \qquad 1\leq i\leq N.
    \end{equation*}
    \item Aplicando recursión:
    \begin{align*}
...
\Ensure{$Q=(q_0,\dots,q_r)$ = secuencia de estados que maximiza $P[\mathcal{X}_0^r=Q|\mathcal{Y}_0^r=O]$}
    ...
    \For{$i\gets1$ \textbf{to} $N$}
        \State $\delta_0(i)\gets \log(b_i(O_0)\cdot\pi_i)$
    \EndFor \State
    
    \LineComment{Calcular $\delta_t(i)$ y $\psi_t(i)$}
    \For{$t\gets1$ \textbf{to} $r$}
        \For{$i\gets1$ \textbf{to} $N$}
            \State $\delta_t(i)\gets \displaystyle\max_{j=1,\dots,N}\left( \delta_{t-1}(j)+\log(a_{ji}) \right)+\log(b_i(O_t))$
...
\section{Reconocimiento de estados}
