...
%Pero también podemos usarlo para determinar los estados de un espacio $\mathcal{S}$ al que sólo se conoce la cardinalidad del espacio, mediante una secuencia de observaciones suficientemente amplia y la aplicación del algoritmo de Baum-Welch, somos capaces de dar un significado físico a los estados.

\begin{exampleth}
Consideramos el ejemplo ilustrado en \cite{Stamp}, el cual pretende estudiar las propiedades básicas del inglés. Para ello, toma el libro \enquote{Brown Corpus} con alrededor de un millón de palabras y considera las primeras $50000$ letras. Tras eliminar las puntuaciones, números y caracteres especiales, convirtió todas las letras a minúscula obteniendo $26$ letras y el espacio, un total de $27$ símbolos al que consideró como posibles observaciones.

Con estas $27$ posibles observaciones y tomando un espacio de estados $\mathbb{S}$ con cardinalidad $N=2$, supone que todas las probabilidades se distribuyen de forma aleatoriamente uniforme, en especial, asigna a cada probabilidad de emisión un valor aleatoriamente cercano a $1/27$. Aplicando $100$ veces el algoritmo de Baum-Welch obtuvo las siguiente probabilidades de emisión:  
...
\begin{longtable}{|c|C|C|C|C|}
        ...
        \hline
        \caption{Probabilidades de emisión antes y después de aplicar el algoritmo de Baum-Welch}
\end{longtable}
Si analizamos estas probabilidades, podemos ver que los estados separan las vocales de los consonantes, de forma que $s_1$ representa el conjunto de vocales y $s_2$ representa el conjunto de consonantes.
