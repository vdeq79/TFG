...
Queremos calcular la probabilidad de secuencia de observaciones concreta, $O=(O_0,O_1,$ $\dots, O_r)$ conocido el modelo. La forma más directa de hacerlo es mediante enumeración de todas las posibles secuencias de estados de longitud $r+1$. Consideramos una de ellas:
...
Por lo tanto, la probabilidad de secuencia de observaciones dada la secuencia de estados $Q$ es:
...
La parte de retroceso del algoritmo no es necesario para resolver \textbf{problema 1}, pero va a ser usada en las soluciones a los \textbf{problemas 2 y 3}, así que vamos a presentarla aquí. 
...
\end{aligned}    
\]
Aplicando la idea de recursión:
\[
\begin{aligned}
...
\begin{figure}[H]
    ...
    main/.style = {draw, circle, minimum size=0.75cm}, dissapear/.style={draw=none}] 
        \node[main] (1) {$s_1$}; 
        ...
        \node[dissapear]  (31) [right of = 30]{$\mathcal{X}_t$};
...
Para llevar la traza de los estados que maximizan \eqref{formulaDeltaViterbi}, definimos $\psi_t(i)$ como el estado en el instante $t-1$ que maximiza $\delta_t(i)$. Resumiendo:
\begin{itemize}
    \item En el instante inicial:
    \begin{align*}
        &\delta_0(i)=b_i(O_0)\cdot\pi_i, && 1\leq i\leq N\\ 
        &\psi_0(i)=0. && 
    \end{align*}
    \item Aplicando recursión:
    \begin{align*}
        &\delta_t(i)=\max_{j=1,\dots,N}\left(a_{ji}\cdot\delta_{t-1}(j)\right) \cdot b_i(O_t), && \\
        &\psi_t(i)=\argmax_{j=1,\dots,N}\left(a_{ji}\cdot\delta_{t-1}(j)\right), && 1\leq t\leq r \\
        & && 1\leq i\leq N
    \end{align*}
    \item Finalmente:
    \begin{align*}
